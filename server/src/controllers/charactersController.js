...
const db = require("../model/character");
    ...
    db.characters
      ...
      .then((characters) => {
        characters.forEach((character) => {
          character.image = `${req.protocol}://${req.get("host")}/characters/${
            character.image
          }`;
          character.dataValues.url = `${req.protocol}://${req.get(
            "host"
        ...
        const response = {
          ...
          characters:
            characters.length > 0
              ? characters
              : "there are no charactes with these conditions",
        };
    ...
    if (!isNaN(req.params.id)) {
      db.characters
        ...
        .then((character) => {
          ...
          character.movies.forEach(
            (movie) =>
              (movie.image = `${req.protocol}://${req.get("host")}/movies/${
                movie.image
              }`)
          );
          character.dataValues.movies.forEach((movie) => {
            movie.dataValues.characterMovie = undefined;
            movie.dataValues.url = `${req.protocol}://${req.get(
              "host"
            )}/movies/${movie.id}`;
          ...
          db.characters
            ...
            .then((result) => {
              ...
              const response = {
                ...
                url: `${req.protocol}://${req.get("host")}/characters/${
                  character.id
                }`,
              };
